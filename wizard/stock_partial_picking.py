...
class stock_partial_picking(osv.osv_memory):
    ...
    _columns = {
        ...
        'picking_id': fields.many2one('m15', 'Picking', required=True, ondelete='CASCADE'),
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if not picking_ids or len(picking_ids) != 1:
            ...
            return res
        assert active_model in ('m15'), 'Bad context propagation'
        picking_id, = picking_ids
        if 'picking_id' in fields:
            res.update(picking_id=picking_id)
        if 'move_ids' in fields:
            picking = self.pool.get('m15').browse(cr, uid, picking_id, context=context)
    ...
    def do_partial(self, cr, uid, ids, context=None):
        ...
        m15.do_partial(cr, uid, [partial.picking_id.id], partial_data, context=context)
